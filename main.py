...
def find_best_path(matrix: [[int]]):
    ...
    step = 0
    while no_improvement_counter < len(matrix) ** 3:
        step += 1
        ...
        if delta > 0 or math.exp(-delta/step) < random.random():
            path = new_path
...
def print_hi(name):
    for case in (small, large):
        path, path_length = find_best_path(case)
        cities_path = list(map(lambda point: cities[point], path))
        print(f"Довжина маршруту: {path_length} км. Маршрут: {'->'.join(cities_path)}")
